...
export default async function AdminErrorDetailPage({ params }: { params: { id: string } }) {
  ...
  if (!error) return <Panel><p className="text-slate-700">找不到錯誤回報。</p></Panel>;
  ...
  return (
    <>
      <PageHeader
        title={error.title}
        description="此頁只顯示已淨化後的錯誤資訊，token、cookie、.env、密碼與敏感資料不會顯示。"
        actions={
          <>
            <LinkButton href="/admin/errors" variant="secondary">返回錯誤中心</LinkButton>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <Panel><p className="text-sm font-bold text-slate-500">狀態</p><div className="mt-2"><StatusBadge label={errorStatusLabels[error.status]} tone={errorStatusTone(error.status)} /></div></Panel>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[1fr_0.8fr]">
        <Panel>
          <h2 className="text-2xl font-black text-slate-950">錯誤摘要</h2>
          <dl className="mt-4 grid gap-3 text-base md:grid-cols-2">
            <div><dt className="font-bold text-slate-700">message（訊息）</dt><dd>{error.message}</dd></div>
            <div><dt className="font-bold text-slate-700">module（模組）</dt><dd>{safeText(error.module, "未提供")}</dd></div>
            <div><dt className="font-bold text-slate-700">route（頁面）</dt><dd>{safeText(error.route, "未提供")}</dd></div>
            <div><dt className="font-bold text-slate-700">action（動作）</dt><dd>{safeText(error.action, "未提供")}</dd></div>
            <div><dt className="font-bold text-slate-700">使用者</dt><dd>{safeText(error.user?.name ?? error.userRole, "未登入或未知")}</dd></div>
            ...
            <div><dt className="font-bold text-slate-700">request id</dt><dd>{safeText(error.requestId, "未提供")}</dd></div>
            <div><dt className="font-bold text-slate-700">session id</dt><dd>{safeText(error.sessionId, "未提供")}</dd></div>
          </dl>
        </Panel>
        ...
        <Panel>
          ...
          <div className="mt-4 grid gap-3">
            <form action={`/api/errors/${error.id}/status`} method="post" className="grid gap-2">
              ...
              <Button type="submit" variant="secondary">更新狀態</Button>
            </form>
            {!fixRequest ? (
      ...
      <Panel className="mt-5">
        <h2 className="text-2xl font-black text-slate-950">sanitized stack trace（已淨化堆疊）</h2>
        <pre className="mt-4 max-h-[420px] overflow-auto rounded-lg bg-slate-950 p-4 text-sm leading-6 text-slate-100">{error.stackTraceSanitized || "未提供 stack trace"}</pre>
      </Panel>
          ...
          <div className="mt-4 grid gap-3">
            {error.breadcrumbs.map((breadcrumb) => (
              <div key={breadcrumb.id} className="rounded-lg border border-slate-200 bg-slate-50 p-3">
                ...
                <p className="mt-1 text-sm text-slate-600">{safeText(breadcrumb.route, "未提供")} · {formatDateTime(breadcrumb.timestamp)}</p>
              </div>
            ))}
            {error.breadcrumbs.length === 0 ? <p className="text-slate-600">尚無操作軌跡。</p> : null}
          </div>
        </Panel>
        <Panel>
          <h2 className="text-2xl font-black text-slate-950">sanitized context（已淨化情境）</h2>
          <pre className="mt-4 max-h-[360px] overflow-auto rounded-lg bg-slate-950 p-4 text-sm leading-6 text-slate-100">{error.contextJsonSanitized || "未提供 context"}</pre>
        </Panel>
      </div>
    </>
  );
